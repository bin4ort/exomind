/* exoqms -> exomind HTTP client: the durable storage backend.
 * Pattern: exosched/src/exomind.c, with shorter socket timeouts (3s) so
 * quality checks stay inside their per-check time budget. */
#include "exoqms.h"
...
/* stores key with a JSON body (avoids exomind's form-parsing when the
 * value contains '=') */
                ...
                char *err, size_t errsz)
{
    ...
    buf_t body = {0};
    buf_printf(&body, "{\"key\":\"%s\",\"value\":\"%s\",\"ttl\":0}", key, je);
    free(je);
    char *resp = NULL;
    size_t rlen = 0;
    int status = 0;
...
/* fetches a key; returns 0 and *out=NULL when the key is missing */
int exo_get(exo_t *e, const char *key, char **out, char *err, size_t errsz)
{
    char target[1024];
    snprintf(target, sizeof target, "/get?key=%s", key);
    char *resp = NULL;
    size_t rlen = 0;
    int status = 0;
    ...
    if (status != 200) {
        snprintf(err, errsz, "exomind get %s failed (status %d)", key,
                 status);
        free(resp);
        return -1;
    }
