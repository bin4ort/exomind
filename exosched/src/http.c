...
const char *http_spec_text(void)
{
    return
        ...
        "## console\n"
        "\n"
        "With no arguments exosched prints this guide and exits. The daemon\n"
        "binds a port only when started with `--serve` (or an explicit\n"
        "`--port`). A path starting with `/` as the first argument runs one\n"
        "API operation in-process - no server, no socket - and prints the\n"
        "answer: `/remind` schedules (POST; body from `--body <text>` or\n"
        "stdin), `/timers` lists active timers, `/timer?id=<id>` cancels:\n"
        "\n"
