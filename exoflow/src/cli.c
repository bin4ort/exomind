/* exoflow -> exomind / exosched HTTP client: the storage backend and the
 * deadline scheduler. Same wire pattern as exosched's exomind.c. */
#include "exoflow.h"
...
int cli_init(cli_t *c, const char *url, char *err, size_t errsz)
{
    ...
    if (strncmp(p, "http://", 7) != 0) {
        snprintf(err, errsz, "bad url (only http://host[:port])");
        return -1;
    }
    p += 7;
    size_t hostlen = 0;
    while (p[hostlen] && p[hostlen] != ':' && p[hostlen] != '/')
        hostlen++;
    if (hostlen == 0 || hostlen >= sizeof c->host) {
        snprintf(err, errsz, "bad url host");
        return -1;
    }
    memcpy(c->host, p, hostlen);
    c->host[hostlen] = 0;
    p += hostlen;
    c->port = 7654;
    if (*p == ':') {
        ...
        if (c->port <= 0 || c->port > 65535) {
            snprintf(err, errsz, "bad url port");
            return -1;
        }
    }
    return 0;
...
static int connect_cli(const cli_t *c, char *err, size_t errsz)
{
    ...
    if (!inet_pton(AF_INET, c->host, &addr.sin_addr)) {
        snprintf(err, errsz, "bad host %s", c->host);
        close(fd);
        return -1;
    }
    if (connect(fd, (struct sockaddr *)&addr, sizeof addr) < 0) {
        snprintf(err, errsz, "connect %s:%d: %s", c->host, c->port,
                 strerror(errno));
        close(fd);
        return -1;
    }
                         ...
                         char *err, size_t errsz)
{
    ...
    if (!hdr_end || h.len < 16) {
        buf_free(&h);
        snprintf(err, errsz, "backend: bad response");
        return -1;
    }
    char *sp = h.p;
    char *eol = memchr(sp, '\n', hdr_end);
    if (!eol) {
        buf_free(&h);
        snprintf(err, errsz, "backend: bad status line");
        return -1;
    }
    *status = 500;
    ...
    if (b.len > EXO_MAX_BODY) {
        buf_free(&b);
        snprintf(err, errsz, "backend: response too large");
        return -1;
    }
...
int cli_request(cli_t *c, const char *method, const char *target,
                ...
                char *err, size_t errsz)
{
    int fd = connect_cli(c, err, errsz);
    if (fd < 0)
        return -1;
    buf_t req = {0};
    buf_printf(&req, "%s %s HTTP/1.1\r\nHost: %s:%d\r\nConnection: close\r\n",
               method, target, c->host, c->port);
    if (body && blen > 0) {
    ...
    if (sent < 0 || (size_t)sent != reqlen) {
        snprintf(err, errsz, "backend: write failed: %s", strerror(errno));
        close(fd);
        return -1;
    }
    char *resp = NULL;
 ...
 * when the value contains '='); ttl 0 = forever */
                ...
                char *err, size_t errsz)
{
    ...
    size_t rlen = 0;
    int status = 0;
    int rc = cli_request(e, "POST", "/set", body.p, body.len, 1, &resp,
...
int exo_del(cli_t *e, const char *key, int *existed, char *err, size_t errsz)
{
    ...
    size_t rlen = 0;
    int status = 0;
    int rc = cli_request(e, "DELETE", target, NULL, 0, 0, &resp, &rlen,
                         &status, err, errsz);
    if (rc != 0)
        return -1;
    if (existed)
...
int exo_note(cli_t *e, const char *text, char *err, size_t errsz)
{
    ...
    size_t rlen = 0;
    int status = 0;
    int rc = cli_request(e, "POST", "/note", text, strlen(text), 0, &resp,
                         &rlen, &status, err, errsz);
    if (rc != 0)
        return -1;
    if (status != 200) {
             ...
             char *err, size_t errsz)
{
    ...
    size_t rlen = 0;
    int status = 0;
    if (cli_request(e, "GET", target, NULL, 0, 0, &resp, &rlen, &status, err,
                    errsz) != 0)
        return -1;
    if (status != 200) {
                  ...
                  char *err, size_t errsz)
{
    ...
    size_t rlen = 0;
    int status = 0;
    int rc = cli_request(e, "POST", "/batch", body.p, body.len, 1, &resp,
                         &rlen, &status, err, errsz);
    buf_free(&body);
    if (rc != 0)
        return -1;
...
/* registers a one-shot reminder on exosched: `at <epoch> "<msg>"`.
 * returns 0 on success, -1 if exosched is unreachable or refused. */
int xs_remind(cli_t *x, int64_t at_epoch, const char *msg,
              char *err, size_t errsz)
{
    char *me = esc_line(msg, strlen(msg));
    buf_t body = {0};
    buf_printf(&body, "at %lld \"%s\"", (long long)at_epoch, me);
    free(me);
    char *resp = NULL;
    size_t rlen = 0;
    int status = 0;
    int rc = cli_request(x, "POST", "/remind", body.p, body.len, 0, &resp,
                         &rlen, &status, err, errsz);
    buf_free(&body);
    if (rc != 0)
        return -1;
    if (status != 200) {
        snprintf(err, errsz, "exosched remind failed (status %d: %s)", status,
                 resp);
        free(resp);
        return -1;
    }
