/* exoflow utilities: memory, buffers, escaping, small hashing. */
#include "exoflow.h"
...
void *xmalloc(size_t n)
{
    void *p = malloc(n ? n : 1);
    if (!p) {
        fprintf(stderr, "exoflow: out of memory\n");
        exit(1);
    }
    return p;
}
...
{
    if (!g_rng)
        g_rng = (uint32_t)((uint64_t)time(NULL) ^ (uint64_t)getpid() * 2654435761u);
    g_rng ^= g_rng << 13;
    g_rng ^= g_rng >> 17;
    g_rng ^= g_rng << 5;
    return g_rng;
